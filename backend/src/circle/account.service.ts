...
export class AccountService {
  ...
  // ---------------------------------------------------------------------------
  // Account building helpers (formerly private in UserOpService)
  // ---------------------------------------------------------------------------

  /**
   * Build a passkey validator from credentials.
   */
  async buildPasskeyValidator(
    client: any,
    credentialId: string,
    publicKey: string,
  ) {
  ...
  buildViemChain(key: string, config: ChainConfig) {
    return defineChain({
      id: config.chainId,
